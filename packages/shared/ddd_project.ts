...
export const DDD_STEP_META: Record<DddStepKind, DddStepMeta> = {
    ...
    [DddStepKind.MiniEventStorming]: {
        ...
        deliverable: "15-25 Domain Events (Vergangenheit), 5-10 Commands, 3-5 externe Systeme, offene Fragen.",
        example:
    ...
    [DddStepKind.UbiquitousLanguage]: {
        ...
        deliverable: "Glossar mit 10-20 Begriffen + kurze Definitionen, konsistent genutzt.",
        example: "Term: Invoice Draft - Eine bearbeitbare Rechnung vor dem Einfrieren / Senden.",
    },
    [DddStepKind.SubdomainsBoundedContexts]: {
        ...
        deliverable: "Subdomänen-Skizze + Mini-Context-Map (Kästen, Pfeile, Beziehungstypen).",
        example: "Subdomains: Billing (Core), Notifications (Supporting), Auth (Generic)\nContexts: BillingContext, NotificationContext, IdentityContext",
    },
    ...
    [DddStepKind.TacticalModel]: {
        ...
        deliverable: "Aggregate-Canvas je Aggregat (Name, Zweck, Invarianten, Commands→Events, Fehlerfälle).",
        example:
    ...
    [DddStepKind.UseCasesPortsContracts]: {
        ...
        deliverable: "3-5 Use-Cases (Vor-/Nachbedingungen, Fehlerpfade) + minimale API/Port Kontrakte.",
        example: "UseCase: AddLineItem (Pre: Draft existiert; Post: neues Item erscheint; Fehler: DraftLocked)\nAPI: POST /invoices/{id}/items",
    },
    [DddStepKind.ArchitectureCut]: {
        label: "Architektur-Schnitt",
        goal: "Minimales Laufsteg-Skelett ohne Over-Engineering festlegen.",
        deliverable: "Ordner-Skeleton nach Kontext(en) & Layern (+ evtl. 1 ADR-Notiz).",
        example: "/src\n  /Billing\n    /domain\n    /application\n    /adapters",
    },
    [DddStepKind.WalkingSkeleton]: {
        ...
        deliverable: "1 Endpoint → Service → Domain → Repository Stub/DB + Tests + Event-Logging.",
        example: "Implemented: POST /invoices (creates draft)\nTests: Accept: create+fetch; Domain: invariant total recalculation.",
    },
    [DddStepKind.ReviewNextSteps]: {
        label: "Review & Next Steps",
        goal: "Lernen festhalten & nächste Schritte planen.",
        deliverable: "Retro (klar vs. wacklig) + Backlog: nächster Slice, Risiken, offene Architekturentscheidungen.",
        example: "Clear: Aggregat 'Invoice' stabil. Uncertain: Zahlungsfluss. Next: Payment Slice evaluieren.",
    },
};
...
>;
export type MiniEventStormingLane = z.infer<
    typeof MiniEventStormingLaneSchema
>;
export type MiniEventStormingCard = z.infer<
    typeof MiniEventStormingCardSchema
>;
export type MiniEventStormingCardType = z.infer<
    typeof MiniEventStormingCardTypeSchema
>;
...
export type DddProjectStepContent = DddProjectContentMap[keyof DddProjectContentMap];
...
export const createNewProject = (
    ...
    return {
        ...
        content: DDD_STEPS.map((kind) => {
            ...
            } else if (kind === DddStepKind.MiniEventStorming) {
                return {
                    kind,
                    lanes: DEFAULT_MINI_EVENT_STORMING_LANES.map((lane) => ({
                        ...lane,
                    })),
                    cards: [],
                    completed: false,
            ...
            } else if (kind === DddStepKind.ThinSlice) {
                return {
                    kind,
                    statement: "",
                    acceptanceCriteria: "",
            ...
            } else if (kind === DddStepKind.ReviewNextSteps) {
                return {
                    kind,
                    retro: "",
                    nextSteps: "",
...
const step = <K extends DddStepKind>(kind: K) => StepBaseSchema.extend({ kind: z.literal(kind) });
...
export const MiniEventStormingCardTypeSchema = z.enum([
    "domain_event",
    "command",
    "policy",
    "external_system",
...
export const MiniEventStormingLaneSchema = z.object({
    id: z.string().min(1),
    title: z.string().min(1),
    category: MiniEventStormingCardTypeSchema,
    description: z.string().optional(),
});

export const MiniEventStormingCardSchema = z.object({
    id: z.string().min(1),
    laneId: z.string().min(1),
    type: MiniEventStormingCardTypeSchema,
    text: z.string(),
...
export const DEFAULT_MINI_EVENT_STORMING_LANES: MiniEventStormingLane[] = [
    {
        id: "domain-events",
        title: "Domain Events",
        category: "domain_event",
        description: "Vergangenheitsform: Was ist passiert?",
    },
    {
        id: "commands",
        title: "Commands",
        category: "command",
        description: "Imperative Auslöser der Events.",
    },
    {
        id: "policies",
        title: "Policies / Regeln",
        category: "policy",
        description: "Welche Regeln/Automationen reagieren?",
    },
    {
        id: "external-systems",
        title: "Externe Systeme",
        category: "external_system",
        description: "Integrationen & Third-Party Systeme.",
    },
    {
        id: "questions",
        title: "Fragen & Hot Spots",
        category: "question",
        description: "Risiken, Wissenslücken, To Clarify.",
    },
] as const satisfies MiniEventStormingLane[];

export const MiniEventStormingContentSchema = step(
    DddStepKind.MiniEventStorming,
).extend({
    lanes: z
        .array(MiniEventStormingLaneSchema)
        .default(DEFAULT_MINI_EVENT_STORMING_LANES),
    cards: z.array(MiniEventStormingCardSchema).default([]),
});
...
export const ThinSliceContentSchema = step(DddStepKind.ThinSlice).extend({
...
export const ReviewNextStepsContentSchema = step(
    DddStepKind.ReviewNextSteps,
).extend({
...
export const DddProjectContentMapSchema = z.object({
    ...
    [DddStepKind.SubdomainsBoundedContexts]: SubdomainsBoundedContextsContentSchema,
